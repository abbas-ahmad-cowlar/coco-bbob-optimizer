...
dimension. On dimensions $2,3,5$ the established baselines lead (best: LMM-CMA-ES,
$\dmf=0.680$); our variants cluster near CMA-ES-2019 ($\dmf\approx0.50$). The decisive finding
...
$86/153$ variant differences are statistically significant.
\end{abstract}
...
$D\in\{2,3,5\}$, instances $1$--$5$, full budget $250\,D$. Dimensions $10$ and $20$ are a
planned extension (Section~\ref{sec:limits}); the pipeline runs them unchanged.
...
checkpoint file, so an interrupted run continues from the last finished unit. The Stage-1 sweep
($1296$ units, $6480$ optimization runs) completed in \textbf{11.6\,h on 4 CPU cores}. Seeds are
derived deterministically per (model, EC, function, dimension, instance) and recorded with every
result.
...
\midrule
LMM-CMA-ES                 & 0.420 & \textbf{0.680} \\
LQ-CMA-ES                  & 0.488 & 0.600 \\
DTS-CMA-ES                 & 0.452 & 0.589 \\
CMA-ES-2019                & 0.357 & 0.524 \\
\midrule
\texttt{pfn\_transformer\_lqEC} (our best) & 0.314 & 0.502 \\
\texttt{gp\_dtsEC}         & 0.313 & 0.499 \\
\textit{remaining 16 variants} & $\sim$0.31 & 0.44--0.50 \\
\bottomrule
\end{tabular}
\end{table}
...
($0.499$) $\approx$ \texttt{gp} ($0.495$) $>$ \texttt{gmm\_np} ($0.468$) $>$
\texttt{bnn\_mc\_dropout} ($0.456$) $>$ \texttt{pfn\_hebo} ($0.451$) $>$ \texttt{pfn\_bnn}
($0.446$). For the evolution control, \texttt{lq} ($0.472$) $\gtrsim$ \texttt{lmm} ($0.468$)
$\gtrsim$ \texttt{dts} ($0.467$), with \texttt{lq} marginally best---consistent with
...
Surrogate accuracy (norm.\ RMSE) & GP $\approx0.036$; neural $0.35$--$0.64$; transformer
$\approx1.0$. \\
How often used / evals saved? & GP saves $\sim$$170$ evals/run (real-eval fraction $0.83$ under
\texttt{dts}); neural $\sim$$80$--$135$; transformer $\sim$$0$. \\
Did the RMSE gate reject bad predictions? & \textbf{Yes, decisively} --- it passes GP and blocks
the transformer. \\
Which EC saves more budget? & \texttt{dts} (real-eval fraction $\approx0.83$--$0.91$) $>$
...
\textbf{$86$ of $153$} variant pairs significantly different at $250\,\fed$: the differences
...
(f10--14, $\dmf=0.909$). Representative convergence (Figure~\ref{fig:conv}) confirms our best
...
\item \textbf{Dimensions $10$ and $20$} are not yet included (this stage is $D\in\{2,3,5\}$).
They are the expensive tier and run unchanged via the resume mechanism; they will shift absolute
numbers but are unlikely to change the qualitative story (GP $>$ neural).
