"""
PFN-HEBO+ surrogate model (practical implementation).

Structured ensemble neural surrogate with feature gating.
Handles irrelevant dimensions better.
"""
...
class _HEBONet(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int):
        super().__init__()

        # Learnable feature gates (dimension relevance)
        self.gates = nn.Parameter(torch.zeros(input_dim))
    ...
    def forward(self, x):
        gate = torch.sigmoid(self.gates)
        x = x * gate  # suppress irrelevant dims
        return self.net(x)


class PFNHEBOSurrogate:
    """PFN-HEBO+ inspired surrogate with feature relevance gating."""
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        if not self.models or self._input_dim != input_dim:
            ...
            self.models = [
                _HEBONet(input_dim, self.hidden_dim).to(train_device)
    ...
    def predict(self, X: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        if not self._fitted:
            raise RuntimeError("PFNHEBOSurrogate.predict called before fit().")
    ...
    def run(self, objective, lb, ub, budget=1000, surrogate_interval=5, n_candidates=0,
        ...
        """CMA-ES loop with PFN-HEBO surrogate and evolution control from Pitra et al. (2021)."""
